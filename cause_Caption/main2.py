...
def print_sample(wordscores, hypotheses, references, test_references, imgs, alphas,losses):
    ...
    for count , (hypothesis, test_reference, tmp_maxes) in enumerate(zip(hypotheses, test_references, wordscores)):
    ...
    df = pd.DataFrame(list(zip(scores,hyp,gt)), columns =['ccc','created validation caption', 'gt'])
    #dfT = df.T
