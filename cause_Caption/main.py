...
def print_sample(max_scores, hypotheses, references, test_references,imgs, alphas,losses):
    ...
    for count , (max_scores,hypothesis, test_reference) in enumerate(zip(max_scores,hypotheses, test_references)):
    ...
    df = pd.DataFrame(list(zip(scores, hyp, gt)), columns =['scores', 'created validation caption', 'gt'])
    #dfT = df.T
...
def validate():
    ...
    print_sample(maxes, hypotheses, references, test_references, all_imgs, all_alphas,losses)
