...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):    
        ...
        #sort_ind = sorted( range(len(caption_lengths)), key=lambda k: caption_lengths[k] )
...
# Load vocabulary
with open('./data/annotations/unique_vocab.pkl', 'rb') as f:
    vocab = pickle.load(f)
...
if from_checkpoint:
    ...
    if torch.cuda.is_available():
        ...
        encoder_checkpoint = torch.load('../checkpoints/base_sort/encoder_epoch0')
        decoder_checkpoint = torch.load('../checkpoints/base_sort/decoder_epoch0')
    ...
    for epoch in tqdm(range(1,num_epochs+1)):
        ...
        if epoch%5==0:    
            torch.save({
                ...
                'loss': loss,
                }, '../checkpoints/causal2/decoder_epoch'+str(epoch))

            torch.save({
                'epoch': epoch,
                'model_state_dict': encoder.state_dict(),
                'loss': loss,
                }, '../checkpoints/causal2/encoder_epoch'+str(epoch))
...
def print_sample(wordscores, hypotheses, references, test_references, imgs, alphas,losses):
    ...
    for count , (hypothesis, test_reference) in enumerate(zip(hypotheses, test_references)):
        ...
        for test in test_reference:
            ...
            #print(test_reference, ref_sentence)
        

        hyp.append(' '.join(hyp_sentence))
        gt.append(' '.join(ref_sentence))
        #scores.append(tmp_maxes)
    
    
    df = pd.DataFrame(list(zip(wordscores,hyp,gt)), columns =['ccc','created validation caption', 'gt'])
    #dfT = df.T
    df.to_csv('validation_captions.csv',index= True)    
    print('made csv')        
...
def visualize(imgs, img_seq_num, alphas, losses, hyp_sentence, smooth=False):
    ...
    for t in range(len(hyp_sentence)):
        ...
        plt.imshow(image)
        print('####t',t)
        current_alpha = alphas[img_seq_num][t, :].cpu().detach().numpy()
...
def validate():
    ...
    for i, (imgs, caps, caplens) in enumerate(tqdm(val_loader)):
        ...
        # Hypotheses
        max_scores, preds = torch.max(scores, dim=2)
        
        preds = preds.tolist()
        temp_preds = list()
        value = max_scores.cpu().detach().numpy()
        
        for j, (p, s) in enumerate( zip(preds, value) ):
            pred = p[:decode_lengths[j]]
            
            tmp_val = s[:decode_lengths[j]]
            #print(tmp_val)
            wordscore.append(tmp_val)        
    ...
    print(wordscore)
    print_sample(wordscore, hypotheses, references, test_references, all_imgs, all_alphas,losses)
